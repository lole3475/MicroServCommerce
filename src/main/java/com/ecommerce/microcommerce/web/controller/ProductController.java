...
import com.ecommerce.microcommerce.viewBean.ProductBean;
...
public class ProductController {
	...
	public ResponseEntity<Void> ajouterProduit(@Valid @RequestBody Product product) {
		...
		if (productAdded.getPrix() == 0)
			throw new ProduitGratuitException("Comment vendre un projet GRATUIT ????");
	...
	public List<String> getProduitsEtMarges() {
		List<Product> ListeProduits = productDao.findAll();
		List<String> ListeFinale = new ArrayList<>();
		for (Product product : ListeProduits) {
			ProductBean pB = new ProductBean(product);
			ListeFinale.add(pB.toString());
		}
		return ListeFinale;
	}

	// Récupérer la liste des produits triée par ordre alphabetique
	@RequestMapping(value = "/Produits/tries", method = RequestMethod.GET)
